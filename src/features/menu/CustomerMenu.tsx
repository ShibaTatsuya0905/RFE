...
const CustomerMenu: React.FC = () => {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([
      apiClient.get('/foods?pageSize=999'),
      apiClient.get('/orders/active'),
      tableId ? apiClient.get(`/tables/${tableId}`) : Promise.resolve({ data: { name: `Bàn ${tableId}` } })
    ])
      .then(([foodsRes, ordersRes, tableRes]) => {
    ...
    <div className="min-h-screen bg-black text-white font-sans flex justify-center pb-28">
      <div className="w-full max-w-md p-4 relative">
        ...
        {customerTab === 'menu' && (
          <>
            {categories.length > 0 && (
              <div className="sticky top-0 z-30 bg-black/90 backdrop-blur-md py-3 -mx-4 px-4 flex overflow-x-auto gap-2.5 no-scrollbar border-b border-slate-800/80">
                {categories.map((cat) => (
