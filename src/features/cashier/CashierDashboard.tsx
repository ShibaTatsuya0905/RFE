...
import { Receipt, Coins, QrCode, Printer, Table as TableIcon, X, Check } from 'lucide-react';
...
const CashierDashboard: React.FC = () => {
  ...
  const handlePay = async (paymentMethod: string) => {
    ...
    try {
      ...
      addToast(`Bàn ${selectedOrder.tableName} đã thanh toán thành công! 💵`, 'success');
  ...
  return (
    <>
      <div className="flex h-screen bg-black text-white font-sans overflow-hidden print:hidden">
        <div className="w-96 bg-gray-900 border-r border-slate-800 flex flex-col">
          <div className="p-6 border-b border-slate-800 flex items-center gap-3">
            ...
            <h1 className="text-xl font-bold">Quầy Thu Ngân</h1>
          </div>
        ...
        <div className="flex-1 bg-black p-8 flex items-center justify-center">
          {selectedOrder ? (
            <div className="bg-gray-900 p-8 rounded-3xl w-full max-w-xl border border-gray-800 shadow-2xl">
                ...
                {selectedOrder.orderDetails.map((item) => (
                  <div key={item.id} className="flex justify-between text-sm">
                    <div className="flex-1">
                      ...
                      {item.notes && <p className="text-xs text-red-400 italic">Ghi chú: {item.notes}</p>}
                    </div>
      ...
      {/* --- PHÂN HỆ IN HÓA ĐƠN GIẤY 80MM (CHỈ HIỂN THỊ KHI BẤM IN) --- */}
      {selectedOrder && (
